...
import { BadgeDetails, BadgeStructure, Member, MemberBadgeProgress } from '@/models/osm';
...
export async function fetchBadgeProgress(accessToken: string, sectionId: string, termId: string, sectionType: string, badgeId: string, badgeVersion: string): Promise<MemberBadgeProgress[]> {
  try {
    // Build the request URL and parameters
